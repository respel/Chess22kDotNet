    ...
    public static class ChessConstants
    {
        ...
        static ChessConstants()
        {
            ...
            for (var i = 0; i < 64; i++)
            {
                ...
                if (i > 55) KingArea[i] |= Util.RightTripleShift(StaticMoves.KingMoves[i], 8);

                if (i < 8) KingArea[i] |= StaticMoves.KingMoves[i] << 8;
            }
            ...
            for (var i = 0; i < 64; i++) KingArea[i] &= ~Util.PowerLookup[i];
        }
    }
}