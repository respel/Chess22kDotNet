...
namespace Chess22kDotNet.Search
{
    public static class NegamaxUtil
    {
        ...
        public static int CalculateBestMove(ChessBoard cb, ThreadData threadData, int ply, int depth,
            ...
            int nullMoveCounter)
        {
            if (!IsRunning) return ChessConstants.ScoreNotRunning;
            ...
            if (depth == 0) return QuiescenceUtil.CalculateBestMove(cb, threadData, alpha, beta);
            ...
            if (Statistics.Enabled) Statistics.AbNodes++;
            ...
            if (!isPv && cb.CheckingPieces == 0)
            {
                ...
                if (EngineConstants.EnableStaticNullMove && depth < StaticNullmoveMargin.Length)
                    ...
                    {
                        if (Statistics.Enabled) Statistics.StaticNullMoved[depth]++;
                ...
                if (EngineConstants.EnableRazoring && depth < RazoringMargin.Length &&
                    ...
                    if (eval + RazoringMargin[depth] < alpha)
                    {
                        ...
                        {
                            if (Statistics.Enabled) Statistics.Razored[depth]++;
                ...
                if (EngineConstants.EnableNullMove)
                        ...
                        MaterialUtil.HasNonPawnPieces(cb.MaterialKey, cb.ColorToMove))
                    {
                        ...
                        if (score >= beta)
                        {
                            if (Statistics.Enabled) Statistics.NullMoveHit++;

                            return score;
                        }

                        if (Statistics.Enabled) Statistics.NullMoveMiss++;
                    }
            }
            ...
            while (phase <= PhaseQuiet)
            {
                switch (phase)
                {
                    case PhaseTt:
                        ...
                        {
                            ttMove = ttEntry.Move;
                            if (cb.IsValidMove(ttMove)) threadData.AddMove(ttMove);
                ...
                while (threadData.HasNext())
                {
                        ...
                        && !cb.IsDiscoveredMove(MoveUtil.GetFromIndex(move)))
                    {
                        if (phase == PhaseQuiet)
                        {
                            ...
                            {
                                if (Statistics.Enabled) Statistics.Lmped[depth]++;
                            ...
                            if (EngineConstants.EnableFutilityPruning && depth < FutilityMargin.Length)
                                ...
                                {
                                    if (eval == Util.ShortMin) eval = EvalUtil.GetScore(cb, threadData);

                                    if (eval + FutilityMargin[depth] <= alpha)
                                    {
                                        if (Statistics.Enabled) Statistics.Futile[depth]++;
                    ...
                    else
                    {
                        ...
                        if (depth > 2 && movesPerformed > 1 && MoveUtil.IsQuiet(move) && !MoveUtil.IsPawnPush78(move))
                        {
                            ...
                            if (threadData.GetMoveScore() > 40) reduction -= 1;

                            if (move == killer1Move || move == killer2Move || move == counterMove) reduction -= 1;
                    ...
                    if (MoveUtil.IsQuiet(move)) threadData.AddBfValue(cb.ColorToMove, move, depth);
                }
            ...
            if (movesPerformed == 0)
            {
                ...
                {
                    if (Statistics.Enabled) Statistics.StaleMateCount++;

                    return EvalConstants.ScoreDraw;
                }

                if (Statistics.Enabled) Statistics.MateCount++;

                return Util.ShortMin + ply;
            }

            if (EngineConstants.Assert) Assert.IsTrue(bestMove != 0);

            // set tt-flag
            var flag = TtUtil.FlagExact;
            if (bestScore >= beta)
                flag = TtUtil.FlagLower;
            else if (bestScore <= alphaOrig) flag = TtUtil.FlagUpper;

            if (IsRunning) TtUtil.AddValue(cb.ZobristKey, bestScore, ply, depth, flag, bestMove);
        ...
        private static int Extensions(ChessBoard cb)
        {
            ...
            if (Statistics.Enabled) Statistics.CheckExtensions++;
